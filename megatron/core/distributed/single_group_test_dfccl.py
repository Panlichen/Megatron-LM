...
def main():
    ...
    group_ranks_list = [
        list(range(0, world_size))  # 全部 GPU 的 ranks
    ]
    ...
    # 确定当前进程所在的组和组内的 rank
    group_id = 0 if rank < world_size else 1
    print(f"Rank: {rank}, World Size: {world_size}, Group ID: {group_id}, len(process_groups): {len(process_groups)}")
    ...
    tensors = [torch.randn(10000).cuda(local_rank).to(torch.float32) for _ in range(1)]
    ...
    for i in range(100):
        ...
        for coll_id, tensor in enumerate(tensors):
            print(f"tensor.numel(): {tensor.numel()} tensor.dtype: {tensor.dtype}, tensor.device: {tensor.device}")
